...
router.get('/stats', requireClient, async (req, res, next) => {
  try {
    ...
    const [total, monthly, tasks] = await Promise.all([
      pool.query('SELECT COALESCE(SUM(hours),0)::numeric AS total FROM time_logs WHERE client_id=$1', [clientId]),
      pool.query('SELECT COALESCE(SUM(hours),0)::numeric AS total FROM time_logs WHERE client_id=$1 AND date >= $2', [clientId, firstOfMonth]),
      pool.query('SELECT COUNT(*)::int AS cnt FROM time_logs WHERE client_id=$1', [clientId]),
    ]);

    res.json({
      totalHours: total.rows[0].total,
      hoursThisMonth: monthly.rows[0].total,
...
router.get('/bookings', requireClient, async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      'SELECT * FROM bookings WHERE client_id=$1 ORDER BY date, time',
      [req.user.id]
    );
...
// ── POST /api/client/bookings — client books a session ───────
