...
router.get('/', requireFreelancer, async (req, res, next) => {
  try {
    const { client_id, from, to, limit = 100, offset = 0, payment_status } = req.query;
...
router.patch('/:id/payment', requireFreelancer, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(`
      UPDATE time_logs SET payment_status = $1
...
router.patch('/bulk-payment', requireFreelancer, async (req, res, next) => {
  try {
    ...
    await pool.query(`
      UPDATE time_logs SET payment_status = $1
      WHERE id = ANY($2) AND freelancer_id = $3
...
router.put('/:id', requireFreelancer, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(`
      UPDATE time_logs SET hours=$1, task_description=$2, date=$3
      WHERE id=$4 AND freelancer_id=$5 RETURNING *
...
router.delete('/:id', requireFreelancer, async (req, res, next) => {
  try {
    await pool.query('DELETE FROM time_logs WHERE id=$1 AND freelancer_id=$2',
